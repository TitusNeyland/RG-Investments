...
/**
 * Helper: Send data back to GHL using their API
 * This allows you to update contact fields or trigger actions in GHL
 */
async function sendToGHL(contactId, data) {
  const ghlApiKey = process.env.GHL_API_KEY;
  const ghlLocationId = process.env.GHL_LOCATION_ID;
  
  if (!ghlApiKey || !ghlLocationId) {
    console.log("GHL API credentials not configured - skipping GHL update");
    return;
  }
...
app.post("/webhook/survey", async (req, res) => {
  try {
    ...
    const contactId = req.body?.contactId || req.body?.contact?.id;
    ...
    // Send response back to GHL if contact ID is available
    if (contactId) {
      await sendToGHL(contactId, { reply });
    }
